...
- `/tmp/security-review-{name}/tech-stack.json` → gates which checks to run
- `/tmp/security-review-{name}/phase2-architecture.json` → auth model, known
  weak areas to prioritize
- `/tmp/security-review-{name}/phase3b-reachability.json` → vulnerable libs
...
LANG=$(cat /tmp/security-review-{name}/tech-stack.json | python3 -c \
  "import sys,json; d=json.load(sys.stdin); print(d['languages'][0])")

semgrep --config="p/${LANG}" --config="p/owasp-top-ten" \
  --json --output /tmp/security-review-{name}/semgrep-raw.json \
...
Write to `/tmp/security-review-{name}/phase4-owasp.json`:
```json
{
