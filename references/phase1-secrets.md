...
```bash
gitleaks detect --source {repo_path} --report-format json --report-path /tmp/security-review-{name}/gitleaks-raw.json --no-git false
```
...
Write to `/tmp/security-review-{name}/phase1-secrets.json`:
```json
{
  "phase": "secrets",
  "summary": {
